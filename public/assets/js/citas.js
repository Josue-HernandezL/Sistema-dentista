...
/* document.addEventListener('DOMContentLoaded', async function () {
  ...
  async function obtenerCitas() {
    try {
      const response = await fetch('https://apis-system-ortodoncist.onrender.com/api/citas', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'x-api-key': 'e4f87c2356032f64d36433baeaf0fd5b' // Reemplaza con tu clave de API
        }
      });
      ...
      // Convierte las citas en un arreglo para FullCalendar
  ...
  // Obtener citas y renderizar el calendario
  const citas = await obtenerCitas();

  const calendar = new FullCalendar.Calendar(calendarEl, {
    events: citas, // Usamos las citas obtenidas de la API
    initialView: 'dayGridMonth',
    headerToolbar: {
      left: 'prev,next today',
      center: 'title',
